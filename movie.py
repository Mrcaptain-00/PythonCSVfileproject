...
def sortingdetails():
    ...
    if ch==1:
        ...
        df.sort_values(['Box_Office_Collection_in_crore'],inplace=True)
        print(df)
    elif ch==5:
        df.sort_values(['Estimated_profit_in_crore'],inplace=True)
        print(df)
    else:
...
def GraphicalRepresentation():
    ...
    Collection=df['Box_Office_Collection_in_crore']
    Buget=df['Buget_in_crore']
    Profit=df['Estimated_profit_in_crore']
    ch=int(input('Enter Your Choice:'))
    if ch==1:
        pt.xlabel('Movie')
